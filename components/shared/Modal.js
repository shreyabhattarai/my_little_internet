...
export default function Modal({ title, icon, onClose, children, closeDelay = MODAL_TRANSITION_MS }) {
      ...
      >
        ...
        {title ? (
          <h2 className={styles.title}>
            {icon ? <span className={styles.titleIcon} aria-hidden="true">{icon}</span> : null}
            {title}
          </h2>
        ) : null}
        {children}
