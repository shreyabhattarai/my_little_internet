export const FOCUS_TRANSITION_MS = 560
export const FOCUS_EASING = "cubic-bezier(0.22, 0.61, 0.36, 1)"
export const FOCUS_TARGET_CENTER = Object.freeze({ x: 0.5, y: 0.5 })
export const BACK_TO_ROOM_LABEL = "Back to room"
export const OVERLAY_LAYOUTS = Object.freeze({
  speaker: {
    base: {
      left: "50%",
      bottom: "12px",
      width: "min(calc(100vw - 24px), 680px)",
      "--speaker-overlay-scale": "1"
    }
  },
  computer: {
    base: {
      left: "45%",
      top: "35%"
    },
    widthVariants: [
      {
        maxWidth: 820,
        left: "47%",
        top: "37%"
      }
    ],
    heightVariants: [
      {
        maxHeight: 760,
        left: "47%",
        top: "38%"
      }
    ]
  },
  arcade: {
    base: {
      left: "50%",
      top: "50%",
      width: "min(calc(100vw - 32px), 760px)"
    }
  }
})

const DEFAULT_OVERLAY_WIDTH_VARIANTS = Object.freeze([
  Object.freeze({ key: "narrow" })
])

const DEFAULT_OVERLAY_HEIGHT_VARIANTS = Object.freeze([
  Object.freeze({ key: "short" })
])

function applyOverlayVariants(resolved, layout, screenSize) {
  const widthVariants = Array.isArray(layout.widthVariants)
    ? layout.widthVariants
    : DEFAULT_OVERLAY_WIDTH_VARIANTS
      .map(({ key }) => layout[key])
      .filter(Boolean)

  const heightVariants = Array.isArray(layout.heightVariants)
    ? layout.heightVariants
    : DEFAULT_OVERLAY_HEIGHT_VARIANTS
      .map(({ key }) => layout[key])
      .filter(Boolean)

  for (const variant of widthVariants) {
    if (typeof variant.maxWidth === "number" && screenSize.width <= variant.maxWidth) {
      Object.assign(resolved, variant)
    }
  }

  for (const variant of heightVariants) {
    if (typeof variant.maxHeight === "number" && screenSize.height <= variant.maxHeight) {
      Object.assign(resolved, variant)
    }
  }
}

function resolveSpeakerBottomInsetPx(screenSize) {
  const height = screenSize.height

  let bottomInset = 12
  if (height <= 420) {
    bottomInset = 8
  } else if (height <= 560) {
    bottomInset = 10
  }

  return bottomInset
}

function resolveSpeakerBottomBandPx(screenSize) {
  const height = screenSize.height

  if (height <= 420) return 160
  if (height <= 560) return 200
  if (height <= 720) return 240
  if (height <= 900) return 280
  return 320
}

function resolveSpeakerOverlayScale(screenSize, bottomInsetPx) {
  const edgeGap = 12
  const topGap = 12
  const speakerGap = 8
  const speakerBottomBand = resolveSpeakerBottomBandPx(screenSize)
  const availableViewportHeight = Math.max(80, screenSize.height - bottomInsetPx - topGap)
  const availableBelowSpeaker = Math.max(80, screenSize.height - speakerBottomBand - bottomInsetPx - speakerGap)
  const availableHeight = Math.min(availableViewportHeight, availableBelowSpeaker)
  const availableWidth = Math.max(120, screenSize.width - edgeGap * 2)

  // Approximate rendered panel size before scale.
  const designWidth = 640
  const designHeight = screenSize.width <= 600 ? 260 : 320

  const widthScale = availableWidth / designWidth
  const heightScale = availableHeight / designHeight
  const fitScale = Math.min(widthScale, heightScale)

  return clamp(fitScale, 0.32, 1)
}

const PHOTO_FRAME_INSET_PRESETS = Object.freeze({
  portrait: Object.freeze({ x: 0.2, y: 0.19, width: 0.6, height: 0.58 }),
  square: Object.freeze({ x: 0.18, y: 0.16, width: 0.64, height: 0.64 }),
  landscape: Object.freeze({ x: 0.16, y: 0.12, width: 0.68, height: 0.72 })
})

export function clamp(value, min, max) {
  return Math.min(max, Math.max(min, value))
}

export function getZoneCenter(zone, fallbackCenter) {
  if (!zone) return fallbackCenter
  return {
    x: zone.x + zone.width / 2,
    y: zone.y + zone.height / 2
  }
}

export function computeCameraShift({
  viewportWidth,
  viewportHeight,
  worldWidth,
  worldHeight,
  activeCenter,
  zoom,
  target = FOCUS_TARGET_CENTER
}) {
  const centerPx = activeCenter
    ? {
        x: (activeCenter.x / worldWidth) * viewportWidth,
        y: (activeCenter.y / worldHeight) * viewportHeight
      }
    : {
        x: viewportWidth / 2,
        y: viewportHeight / 2
      }

  const targetPxX = viewportWidth * target.x
  const targetPxY = viewportHeight * target.y
  const rawShiftX = activeCenter ? targetPxX - centerPx.x * zoom : 0
  const rawShiftY = activeCenter ? targetPxY - centerPx.y * zoom : 0

  const minShiftX = viewportWidth - viewportWidth * zoom
  const minShiftY = viewportHeight - viewportHeight * zoom

  return {
    shiftX: clamp(rawShiftX, minShiftX, 0),
    shiftY: clamp(rawShiftY, minShiftY, 0)
  }
}

export function resolveOverlayAnchorStyle(kind, screenSize) {
  const layout = OVERLAY_LAYOUTS[kind]
  if (!layout) return {}

  const resolved = { ...layout.base }

  applyOverlayVariants(resolved, layout, screenSize)

  if (kind === "speaker") {
    const bottomInsetPx = resolveSpeakerBottomInsetPx(screenSize)
    resolved.bottom = `${bottomInsetPx}px`
    delete resolved.top
    resolved["--speaker-overlay-scale"] = resolveSpeakerOverlayScale(screenSize, bottomInsetPx).toFixed(3)
  }

  delete resolved.maxWidth
  delete resolved.maxHeight

  return resolved
}

export function resolveBackToRoomButtonStyle({
  screenSize,
  viewportSize,
  stageSize,
  worldSize,
  camera,
  zone
}) {
  const isCompact = screenSize.width <= 820
  const edgeGap = isCompact ? 10 : 14
  const topGap = isCompact ? 40 : 46
  const halfButtonWidth = isCompact ? 52 : 58

  const style = {
    left: "50%",
    top: isCompact ? "12px" : "14px",
    transform: "translate(-50%, 0)",
    fontSize: isCompact ? "11px" : "12px",
    padding: isCompact ? "6px 12px" : "7px 14px"
  }

  if (!zone || !camera || !viewportSize || !stageSize || !worldSize) {
    return style
  }

  const zoneAnchorX = zone.x + zone.width / 2
  const zoneAnchorY = zone.y
  const pointX = (zoneAnchorX / worldSize.width) * viewportSize.width
  const pointY = (zoneAnchorY / worldSize.height) * viewportSize.height

  const projectedX = pointX * camera.zoom + camera.shiftX
  const projectedY = pointY * camera.zoom + camera.shiftY
  const stageLeft = (screenSize.width - stageSize.width) / 2
  const stageTop = (screenSize.height - stageSize.height) / 2

  const anchorX = clamp(projectedX + stageLeft, edgeGap + halfButtonWidth, screenSize.width - edgeGap - halfButtonWidth)
  const anchorY = clamp(projectedY + stageTop - (isCompact ? 10 : 12), topGap, screenSize.height - edgeGap)

  style.left = `${Math.round(anchorX)}px`
  style.top = `${Math.round(anchorY)}px`
  style.transform = "translate(-50%, -100%)"

  return style
}

export function resolveWindowControlsStyle(screenSize) {
  const minEdgeGap = 12
  const idealWidth = Math.min(screenSize.width * 0.72, 640)
  const maxWidthFromScreen = Math.max(240, screenSize.width - minEdgeGap * 2)
  const width = clamp(idealWidth, 240, maxWidthFromScreen)

  const style = {
    left: "50%",
    top: "50%",
    width: `${Math.round(width)}px`,
    transform: "translate(-50%, -50%)"
  }

  if (screenSize.width <= 920) {
    style.width = `${Math.round(clamp(screenSize.width * 0.82, 260, maxWidthFromScreen))}px`
  }

  if (screenSize.width <= 640) {
    style.width = `${Math.round(clamp(screenSize.width * 0.9, 260, maxWidthFromScreen))}px`
  }

  if (screenSize.height <= 620) {
    style.top = "52%"
  }

  return style
}

export function resolvePhotoFrameInset(zone, frameImage) {
  if (!zone) return null

  const ratio = frameImage?.width && frameImage?.height
    ? frameImage.width / frameImage.height
    : zone.width / Math.max(1, zone.height)

  const preset = ratio >= 1.2
    ? PHOTO_FRAME_INSET_PRESETS.landscape
    : ratio <= 0.9
      ? PHOTO_FRAME_INSET_PRESETS.portrait
      : PHOTO_FRAME_INSET_PRESETS.square

  return {
    x: zone.x + zone.width * preset.x,
    y: zone.y + zone.height * preset.y,
    width: zone.width * preset.width,
    height: zone.height * preset.height
  }
}
