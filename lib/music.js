...
export function getDefaultPlaylistId(sourcePlaylists = playlists) {
  if (!Array.isArray(sourcePlaylists) || sourcePlaylists.length === 0) {
    return ""
  }

  const preferred = sourcePlaylists.find((playlist) => playlist.id === DEFAULT_PLAYLIST_ID)
  return preferred?.id || sourcePlaylists[0].id
}

export function pickRandomTrackFromPlaylist(sourcePlaylists = playlists, playlistId = DEFAULT_PLAYLIST_ID) {
  if (!Array.isArray(sourcePlaylists) || sourcePlaylists.length === 0) {
    return null
  }

  const selectedPlaylist =
    sourcePlaylists.find((playlist) => playlist.id === playlistId) || sourcePlaylists[0]
  ...
  return {
    ...
    track: selectedPlaylist.tracks[randomIndex]
  }
}
